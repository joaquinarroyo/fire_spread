...
Fire simulate_fire(
  const std::vector<Cell> landscape,
  size_t n_row,
  size_t n_col,
  ...
  while (burning_size > 0) {
    ...
    std::vector<size_t> burned_ids_0_all;
    std::vector<size_t> burned_ids_1_all;
    size_t end_forward_all = 0;

    #pragma omp parallel
    {
      // local variables
      std::vector<size_t> local_ids_0;
      std::vector<size_t> local_ids_1;
      size_t local_forward = 0;

      // Loop over burning cells in the cycle
      // b is going to keep the position in burned_ids that have to be evaluated in this burn cycle
      #pragma omp for reduction(+:processed_cells) schedule(dynamic, 4)
      for (size_t b = start; b < end; b++) {
        ...
        // [II] Vectorizado a nivel de operacion individual
        for (size_t n = 0; n < 8; ++n) {
        ...
        for (size_t n = 0; n < 8; ++n) {
          if (burn_flags[n]) {
            ...
            local_ids_0.push_back(neighbors_coords_0[n]);
            local_ids_1.push_back(neighbors_coords_1[n]);
            local_forward++;
          }
        }
      }

      // Al final, mergear los resultados en sección crítica
      #pragma omp critical
      {
        burned_ids_0_all.insert(burned_ids_0_all.end(), local_ids_0.begin(), local_ids_0.end());
        burned_ids_1_all.insert(burned_ids_1_all.end(), local_ids_1.begin(), local_ids_1.end());
        end_forward_all += local_forward;
      }
    }
    burned_ids_0.insert(burned_ids_0.end(), burned_ids_0_all.begin(), burned_ids_0_all.end());
    burned_ids_1.insert(burned_ids_1.end(), burned_ids_1_all.begin(), burned_ids_1_all.end());
    end_forward += end_forward_all;
